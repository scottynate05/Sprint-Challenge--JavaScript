...
const universities = [];
graduates.map(schoolName => {
  universities.push(schoolName.university)
});
...
graduates.filter(hasUni => {
  unisWithUni.push(hasUni.university.includes("Uni"))
  return hasUni;
})
...
zooAnimals.map(lower => {
  lowCaseAnimalNames.push(lower.animal_name.toLowerCase())
})
...
zooAnimals.filter(pop => {
  lowPopulationAnimals.push(pop.population < 5)
})
...
*/
const populationTotal = zooAnimals.reduce((callback, totalPop) => {
  return (callback, totalPop.population)
})
